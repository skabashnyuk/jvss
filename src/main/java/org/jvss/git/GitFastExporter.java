...
public class GitFastExporter {
    ...
    private final PrintStream git;

    public GitFastExporter(Logger logger, RevisionAnalyzer revisionAnalyzer, ChangesetBuilder changesetBuilder,
                           PrintStream git)
    {
        this.git = git;
        this.logger = logger;
    ...
    public void exportToGit(String repoPath)
    {
        ...
        for (Changeset changeset : changesets)
        {
            ...
            if (labels.size() > 0)
            {
                ...
                {
                    String labelName = ((VssAction.VssLabelAction)label.getAction()).getLabel();
    ...
    {
        boolean needCommit = false;
        VssAction.VssActionType actionType = revision.getAction().type();
        if (revision.getItem().isProject())
        {
            ...
            if (revision.getAction() instanceof VssAction.VssNamedAction)
            {
                VssAction.VssNamedAction namedAction = (VssAction.VssNamedAction)revision.getAction();
                if (namedAction != null)
                {
            ...
            VssPathMapper.VssItemInfo itemInfo = null;
            switch (actionType)
            {
                case Label :
                ...
                case Destroy : {
                    ...
                    if (targetPath != null && !itemInfo.isDestroyed())
                    {
                        if (target.isProject())
                        {
                            ...
                            {
                                if (((VssPathMapper.VssProjectInfo)itemInfo).containsFiles())
                                {
                ...
                case Rename : {
                    VssAction.VssRenameAction renameAction = (VssAction.VssRenameAction)revision.getAction();
                    ...
                    if (targetPath != null && !itemInfo.isDestroyed())
                    {
                        ...
                        if (target.isProject() ? IoUtil.isExists(sourcePath) : IoUtil.isExists(sourcePath))
                        {
                            ...
                            if (itemInfo instanceof VssPathMapper.VssProjectInfo)
                            {
                                VssPathMapper.VssProjectInfo projectInfo = (VssPathMapper.VssProjectInfo)itemInfo;
                ...
                {
                    VssAction.VssMoveFromAction moveFromAction = (VssAction.VssMoveFromAction)revision.getAction();
                    ...
                    VssPathMapper.VssProjectInfo projectInfo =
                ...
                case MoveTo : {
                    ...
                    VssAction.VssMoveToAction moveToAction = (VssAction.VssMoveToAction)revision.getAction();
                    //                       logger.WriteLine("%s: Move to %s from %s",
                    //                           projectDesc, moveToAction.NewProject, targetPath ?? target.LogicalName);
                    VssPathMapper.VssProjectInfo projectInfo = pathMapper.moveProjectTo(project, target, moveToAction.getNewProject());
                ...
                case Pin : {
                    VssAction.VssPinAction pinAction = (VssAction.VssPinAction)revision.getAction();
                ...
                case Branch : {
                    VssAction.VssBranchAction branchAction = (VssAction.VssBranchAction)revision.getAction();
                    logger.WriteLine(String.format("%s: %s %s", projectDesc, actionType, target.getLogicalName()));
                ...
                case Archive :
                    // currently ignored
                {
                    VssAction.VssArchiveAction archiveAction = (VssAction.VssArchiveAction)revision.getAction();
                ...
                case Restore : {
                    VssAction.VssRestoreAction restoreAction = (VssAction.VssRestoreAction)revision.getAction();
            ...
            if (targetPath != null)
            {
                ...
                if (writeProject && pathMapper.isProjectRooted(target.getPhysicalName()))
                {
                    ...
                    for (VssPathMapper.VssProjectInfo projectInfo : pathMapper.getAllProjects(target.getPhysicalName()))
                    {
                    ...
                    for (VssPathMapper.VssFileInfo fileInfo : pathMapper.getAllFiles(target.getPhysicalName()))
                    {
        ...
        else if (actionType == VssAction.VssActionType.Edit || actionType == VssAction.VssActionType.Branch)
        {
    ...
    private boolean writeRevision(VssPathMapper pathMapper, VssAction.VssActionType actionType, String physicalName, int version,
